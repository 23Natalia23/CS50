...
int main(int argc, char *argv[])
{
    ...
    if (n < 1 || n > 100)
    {
        fprintf(stderr, "Positive integer <= 100.\n");
        return 2;
    }
    ...
    int in_padding =  (4 - (orig_Width * sizeof(RGBTRIPLE)) % 4) % 4;
    
    // change width and height of image by n
    bi.biWidth *= n;
    bi.biHeight *= n;
    ...
    // change total size of image
    bi.biSizeImage = ((sizeof(RGBTRIPLE) * bi.biWidth)
    + out_padding) * abs(bi.biHeight);
    ...
    for (int i = 0, biHeight = abs(bi.biHeight); i < biHeight; i++)
    {
        ...
        long offset = orig_Width * sizeof(RGBTRIPLE) + in_padding;
        if ((i + 1) % n != 0)
        {
