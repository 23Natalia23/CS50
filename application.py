...
@login_required
def index():
    # select each symbol and it's amount
    ...
    # update each symbol, price and total
...
def register():
    ...
    if request.method == "POST":
        ...
        # insert a new user into users, storing the hash of the user's password
...
def sell():
    ...
    if request.method == "GET":
        ...
        # update history of selling
        db.execute("INSERT INTO histories (symbol, shares, price, id) \
                    VALUES(:symbol, :shares, :price, :id)", \
                    symbol=stock["symbol"], shares=-shares, \
                    price=usd(stock["price"]), id=session["user_id"])
                       
        # update user cash  by increasing        
        db.execute("UPDATE users SET cash = cash + :purchase WHERE id = :id", \
                    id=session["user_id"], \
                    purchase=stock["price"] * float(shares))
        ...
        # if above shares_total is zero, delete shares from portfolio
        if shares_total == 0:
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if loan < 0:
                ...
                return apology("Cannot loan more than $500 at once!")
        except:
            return apology("Must be positive!")
